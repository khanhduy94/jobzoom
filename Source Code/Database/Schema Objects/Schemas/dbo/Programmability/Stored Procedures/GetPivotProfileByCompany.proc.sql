...
BEGIN
	...
	SET @SQLString = N'CREATE VIEW [' + @viewName + '] AS
					SELECT row_number() over (order by pvt.ProfileBasicId desc) as ID, pvt.* FROM (
						SELECT [Profile.Basic].ProfileBasicId, [Profile.Basic].UserId, [Job.Posting].CompanyId, [Job.Posting].CompanyName, [Job.Approval].JobPostingId, [Job.Posting].JobTitle as JobName, [Job.Approval].IsApproved, TagAttribute.TagName
						FROM [Profile.Basic]
